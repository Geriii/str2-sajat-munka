...
const checkValues = (array) => array.map(row => {
    return (row[0]==="x" && row[1]==="x" && row[2]==="x") || (row[0]==="0" && row[1]==="0" && row[2]==="0");
    })
    .indexOf(true) !== -1;
    /*
...
const setMessage = (message) => {
    document.querySelector("div.message").innerHTML = message;
}
...
const newGame = () => {
    ...
    .addEventListener("click", newGameClickHandler);
};
    /*
